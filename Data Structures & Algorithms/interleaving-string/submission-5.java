class Solution {
    ...
    public boolean helper(String s, String t, String st, int i, int j, int k , int[][] dp) {
        ...
        if (dp[i][j] != 0) return dp[i][j] == 1;
            ...
            result = helper(s, t, st, i + 1, j, k + 1, dp)
                || helper(s, t, st, i, j + 1, k + 1, dp);
        }
        else if (i != s.length() && s.charAt(i) == st.charAt(k)) {
            result = helper(s, t, st, i + 1, j, k + 1, dp);
        }
        else if (j != t.length() && t.charAt(j) == st.charAt(k)) {
            result = helper(s, t, st, i, j + 1, k + 1, dp);
        }
