class Solution {
    public int rob(int[] nums) {
        ...
        // As circluar, call dp twice once with first house and once with last house
        return Math.max(
    ...
    public int dp(int[] nums, int index, int end, int[] memo) {
        ...
        result = Math.max(
            dp(nums, index + 1, end, memo), // House skipped
            nums[index] + dp(nums, index + 2, end, memo) // House robbed
            );
